...
public class SJFFX extends Application {
	...
	public void start(Stage primaryStage) {
		...
		do {
			...
			while (!validInput) {
				try {
					...
					System.out.println();
					System.out.println("Please enter a numerical number only.");
					sc.nextLine();
				}
			}
		...
		System.out.println();
		System.out.println("Showing the Gantt Chart Table in 2 seconds...");
		System.out.println("");
		System.out.println("Created by Arrel R. Bantilan");
		System.out.println("From: BSCpE-2A | Task Performance in Operating System (Finals)");
	...
	private void displayAverageTimes(float totalTime, float averageWaitingTime, float averageTurnaroundTime) {
		System.out.println();
		System.out.printf("Total Time is: %.0f%n", totalTime);
		System.out.println();
		System.out.printf("Average Waiting Time is: %.2fms%n", averageWaitingTime);
		System.out.printf("Average Turnaround Time is: %.2fms%n", averageTurnaroundTime);
	}
	...
	private void launchGanttChartWindow(Stage primaryStage, int[] completionTimes, int totalTime,
		...
		Text totalTimeText = new Text("Total Time is: " + totalTime);
		totalTimeText.setFont(new Font("Verdana", 12));
		totalTimeText.setX(30);
		totalTimeText.setY(120);

		Text avgWaitingTimeText = new Text("Average Waiting Time is: " + String.format("%.2fms", averageWaitingTime));
		avgWaitingTimeText.setFont(new Font("Verdana", 12));
		avgWaitingTimeText.setX(30);
		avgWaitingTimeText.setY(140);

		Text avgTurnaroundTimeText = new Text(
				"Average Turnaround Time is: " + String.format("%.2fms", averageTurnaroundTime));
		avgTurnaroundTimeText.setFont(new Font("Verdana", 12));
		...
		primaryStage.setTitle("Print | Shortest Job First");
